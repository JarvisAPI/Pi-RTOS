 ...
 *
 *	We use the FreeRTOS to save context before entering the vectorising ISR, 
 *  and emitting the true ISR event.
 *
 **/

extern void rpi_irq_service_routine(void);

void vFreeRTOS_ISR( void ) __attribute__((naked));
void vFreeRTOS_ISR( void ) {
	portSAVE_CONTEXT();
	rpi_irq_service_routine();
	portRESTORE_CONTEXT();	
}
